...
uniform vec4 u_Color;
uniform vec3 u_LightColor;
uniform vec3 u_LightDirection; //IS USED HERE! Not used in this shader, but included to avoid errors when we query for it's location
uniform vec3 u_LightPosition;
uniform float u_Specularity;
uniform float u_cutOff;           //NEW UNIFORMS
uniform float u_outerCutOff;      //NEW UNIFORMS
...
vec3 PointLight(
    in vec3 position,
    in vec3 color,
    in vec3 direction, //NEW PARAMETER
    in float constant,
    in float linear,
    in float quadratic,
    in float cutOff,
...
)
{
    ...
    float attenuation = 1.0 / (constant + linear * distance + quadratic * distance * distance + 1.0);
...
void main()
{
vec3 pointLight = PointLight(u_LightPosition,u_LightColor,u_LightDirection,1.0,0.35,0.44,u_cutOff,u_outerCutOff);
